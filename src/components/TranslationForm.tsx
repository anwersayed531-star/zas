...
export const TranslationForm = () => {
  ...
  const { setSourceCode, setTranslatedCode } = useCode();
  const { user } = useAuth();
  ...
  // Update CodeContext whenever input or translated content changes
  useEffect(() => {
    setSourceCode(input);
  }, [input, setSourceCode]);

  useEffect(() => {
    setTranslatedCode(translatedContent);
  }, [translatedContent, setTranslatedCode]);
      ...
      if (user) {
        try {
          await supabase.from('translation_history').insert({
            ...
            translated_content: translationMode === 'multi' ? data.translations : { [targetLangs[0]]: data.translatedCode },
          });
        } catch (saveError) {
