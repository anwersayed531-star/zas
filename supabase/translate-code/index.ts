...
// RTL languages list
const RTL_LANGUAGES = ['arabic', 'عربية', 'hebrew', 'עברית', 'persian', 'فارسی', 'urdu', 'اردو', 'ar', 'he', 'fa', 'ur', 'farsi', 'پارسی'];
...
function injectTexts(html: string, originalTexts: TextNode[], translatedTexts: string[]): string {
  ...
  for (let i = originalTexts.length - 1; i >= 0; i--) {
    ...
    if (original.type === 'attribute' && original.attributeName) {
      ...
      // For text nodes, find and replace the exact text (with whitespace handling)
      // We need to be careful not to replace texts in tags or attributes
      const textPattern = new RegExp(
        `>\\s*${original.text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*<`, 
        'g'
      );
...
/**
 * Update lang attribute in HTML tag based on target language
 */
function updateLangAttribute(html: string, targetLang: string): string {
  // Map language names to ISO 639-1 codes
  const langCodeMap: { [key: string]: string } = {
    'english': 'en', 'arabic': 'ar', 'turkish': 'tr', 'spanish': 'es',
    'french': 'fr', 'german': 'de', 'italian': 'it', 'chinese': 'zh',
    'japanese': 'ja', 'korean': 'ko', 'russian': 'ru', 'hindi': 'hi',
    'persian': 'fa', 'urdu': 'ur', 'polish': 'pl', 'thai': 'th',
    'swedish': 'sv', 'greek': 'el', 'czech': 'cs', 'romanian': 'ro',
    'hungarian': 'hu', 'finnish': 'fi', 'ukrainian': 'uk', 'malay': 'ms',
    'filipino': 'tl', 'indonesian': 'id', 'bengali': 'bn', 'portuguese': 'pt',
    'dutch': 'nl', 'vietnamese': 'vi', 'hebrew': 'he'
  };
  
  const langCode = langCodeMap[targetLang.toLowerCase()] || targetLang.toLowerCase().slice(0, 2);
  
  console.log(`🌐 Updating lang attribute to: ${langCode}`);
  
  // Update lang in <html> tag
  const htmlTagRegex = /<html([^>]*)>/i;
  const match = html.match(htmlTagRegex);
  ...
  if (match) {
    ...
    if (attrs.includes('lang=')) {
      ...
      return html.replace(/<html([^>]*)lang=["'][^"']*["']/, `<html$1lang="${langCode}"`);
    } else {
      // Add lang attribute
      return html.replace(htmlTagRegex, `<html${attrs} lang="${langCode}">`);
    }
  }
...
- Each element in the output array corresponds to the same position in the input array
...
Input array has ${texts.length} texts. Your output MUST also have ${texts.length} texts.`
        },
        {
          role: 'user',
  ...
  try {
    ...
    if (parsed.length !== texts.length) {
      console.error(`⚠️ Length mismatch: expected ${texts.length}, got ${parsed.length}`);
      console.error('Original texts:', texts);
      console.error('Translated texts:', parsed);
      throw new Error(`Translation count mismatch: expected ${texts.length} but got ${parsed.length}`);
    }
    
    console.log(`✅ Successfully translated ${parsed.length} texts`);
    return parsed;
  } catch (e) {
    console.error('Failed to parse Groq response:', content);
    const errorMsg = e instanceof Error ? e.message : 'Unknown error';
    throw new Error(`Invalid JSON response from Groq: ${errorMsg}`);
  }
}
...
${JSON.stringify(texts)}`
  ...
  try {
    ...
    const errorMsg = e instanceof Error ? e.message : 'Unknown error';
    throw new Error(`Invalid JSON response from Google AI: ${errorMsg}`);
  }
}
...
Input array has ${texts.length} texts. Your output MUST also have ${texts.length} texts.`
  ...
  try {
    ...
    const errorMsg = e instanceof Error ? e.message : 'Unknown error';
    throw new Error(`Invalid JSON response from Cloudflare AI: ${errorMsg}`);
  }
}
...
async function translateCode(code: string, sourceLang: string, targetLang: string): Promise<TranslationResponse> {
  try {
    ...
    // Log untranslated texts for debugging
    const untranslatedCount = translatedTexts.filter((text, idx) => text === textsToTranslate[idx]).length;
    if (untranslatedCount > 0) {
      console.warn(`⚠️ Found ${untranslatedCount} untranslated texts out of ${translatedTexts.length}`);
      translatedTexts.forEach((text, idx) => {
        if (text === textsToTranslate[idx]) {
          console.warn(`  - Untranslated: "${text}"`);
        }
      });
    }

    // Step 4: Update lang attribute
    console.log('🌐 Step 4: Updating lang attribute...');
    translatedCode = updateLangAttribute(translatedCode, targetLang);

    // Step 5: Add RTL support if needed
    console.log('📝 Step 5: Applying RTL directive if needed...');
